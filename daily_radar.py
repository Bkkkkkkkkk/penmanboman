...
if __name__ == "__main__":
    try:
        ...
        if not daily_df.empty:
            ...
            # 建立從第一天到「昨天」的完整日曆
            full_dates = pd.date_range(start=min_date, end=yesterday_pd)
            # 重新索引，沒花錢的日子自動補 0
        ...
        daily_df['Rolling_Mean'] = daily_df['Amount'].rolling(window=30, min_periods=1).mean()
        daily_df['Rolling_Std'] = daily_df['Amount'].rolling(window=30, min_periods=1).std().fillna(1)
        # 避免標準差為 0 導致 Z-score 算出無限大 (inf)
        daily_df['Rolling_Std'] = daily_df['Rolling_Std'].replace(0, 1)
        daily_df['Z_Score'] = (daily_df['Amount'] - daily_df['Rolling_Mean']) / daily_df['Rolling_Std']
        
        # 抓取「昨日」特徵
        if yesterday_pd in daily_df['Date'].values:
            target_row = daily_df[daily_df['Date'] == yesterday_pd].iloc[0]
